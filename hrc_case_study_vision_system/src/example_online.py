...
def main():
    ...
    try:
        (trans, rot) = listener.lookupTransform(camera_link_name,
                                                frame_to_get,
                                                rospy.Time(0))
    ...
    P_camera_obj = M_robot_camera[0:3, -1]
    M_robot_obj = np.dot(M_robot_camera, P_camera_obj)
    print(f"M_robot_obj:\n {M_robot_obj}")
